...
for col, label in headers:
    batch.append(gspread.Cell(3, col, label))

# Row 2: 計算式の注記
batch.append(gspread.Cell(2, 12, '=TERRA実入り+GL税抜×1.1+FT税抜×1.1+直契約税込-岡本税抜-木原税抜'))
...
for pm in pay_months:
    for bucket in ['15','matsu']:
        ...
        gl_incl=int(gl_v*1.1); ft_incl=int(ft_v*1.1)
    ...
    s_gl_incl=int(s_gl*1.1); s_ft_incl=int(s_ft*1.1)
...
# ================================================================
print("\n" + "="*100)
print("入金予測 確定版 — 各項目の税込/税抜を明記")
print("="*100)
print()
print("【計算式】")
print("  総実入り = TERRA実入り + GL(税抜)×1.1 + FT(税抜)×1.1 + 直契約(税込) - 岡本(税抜) - 木原(税抜)")
print()
print(f"{'月':>4} | {'TR請求':>7} | {'源泉':>6} | {'TR税込':>7} | {'TR実入':>8} | {'GL税抜':>7} | {'GL税込':>7} | {'FT税抜':>8} | {'FT税込':>8} | {'直契約':>7} | {'岡本':>8} | {'木原':>7} | {'総実入':>9}")
print(f"{'':>4} | {'(税抜)':>7} |        | {'':>7} |          | {'':>7} | {'(×1.1)':>7} | {'':>8} | {'(×1.1)':>8} | {'(税込)':>7} | {'(税抜)':>8} | {'(税抜)':>7} |")
print("-"*130)
...
for pm in pay_months:
    ...
    y,m=pm//100,pm%100
    gl_z = vals[7]; gl_i = int(gl_z*1.1)
    ft_z = vals[8]; ft_i = int(ft_z*1.1)
    ch = vals.get(9,0)
    
    print(f"{m}月".rjust(4) + 
          f" | {vals[3]:>7,} | {vals[4]:>6,} | {vals[5]:>7,} | {vals[6]:>8,}" +
          f" | {gl_z:>7,} | {gl_i:>7,}" +
          f" | {ft_z:>8,} | {ft_i:>8,}" + 
          f" | {ch:>7,} | {vals[10]:>8,} | {vals[11]:>7,} | {vals[12]:>9,}")
    ann_sou += vals[12]

print("-"*130)
print(f"年間{'':>56} {'':>16} {'':>16} {'':>7} {'':>8} {'':>7} | {ann_sou:>9,}")
print(f"\n月平均総実入り: {ann_sou//12:,}")
