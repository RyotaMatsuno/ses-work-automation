...
print('OpenAI:', r.status_code, r.json()['choices'][0]['message']['content'] if r.status_code==200 else r.json().get('error',{}).get('message','')[:100])
...
print('Gemini:', r2.status_code, r2.json()['candidates'][0]['content']['parts'][0]['text'] if r2.status_code==200 else r2.json().get('error',{}).get('message','')[:100])
