...
from freee_auth.token_manager import get_access_token, get_headers
...
if r.status_code == 200:
    ...
    for inv in invoices[:3]:
        print(f"  id:{inv.get('id')} 宛先:{inv.get('partner_display_name')} ステータス:{inv.get('invoice_status')} 発行日:{inv.get('invoice_date')}")
    # PDFダウンロードAPIを試す
    if invoices:
