...
# 取引先一覧（最初の3件）
r2 = requests.get(f"https://api.freee.co.jp/api/1/partners?company_id={company_id}&limit=3", headers=headers, timeout=15)
print(f"partners: {r2.status_code} ({len(r2.json().get('partners',[]))}件)")

# 請求書一覧（最初の3件）
r3 = requests.get(f"https://api.freee.co.jp/api/1/invoices?company_id={company_id}&limit=3", headers=headers, timeout=15)
