...
def load_token():
    ...
    if int(time.time()) - saved_at > expires_in - 300:
        res = requests.post("https://accounts.secure.freee.co.jp/public_api/token", data={
...
def create_invoice_draft(entry, issue_date, due_date, target_mon_str):
    ...
    if res.status_code in (200, 201):
        ...
        print(f"  OK | {entry['name']} / {entry['partner']} / {entry['seikyu']:,}円 | {entry['rule']} → #{inv['invoice_number']}")
        return True
    else:
...
def run(target_month):
    ...
    ok = ng = 0
    for e in entries:
        if create_invoice_draft(e, issue_date, due_date, target_mon_str): ok += 1
        else: ng += 1
