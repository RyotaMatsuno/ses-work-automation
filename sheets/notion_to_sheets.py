    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            if not os.path.exists(CREDENTIALS_PATH):
                raise FileNotFoundError(f"credentials.jsonが見つかりません: {CREDENTIALS_PATH}")
...
def project_to_row(page: dict) -> list:
    ...
    name = extract_title(props, "案件名") or extract_title(props, "タイトル") or extract_title(props, "名前")
    status = extract_select(props, "ステータス") or extract_select(props, "状態")
    skills = extract_multi_select(props, "必須スキル") or extract_multi_select(props, "スキル")
    budget = extract_number(props, "予算（万円）") or extract_number(props, "単価（万円）")
    start = extract_date(props, "開始日") or extract_date(props, "稼働開始日")
    end = extract_date(props, "終了日") or extract_date(props, "稼働終了日")
    client = (
        extract_rich_text(props, "クライアント")
...
def write_sheet(sheets_service, spreadsheet_id: str, sheet_name: str, headers: list, rows: list):
    ...
    sheets_service.spreadsheets().values().update(
        spreadsheetId=spreadsheet_id, range=f"{sheet_name}!A1", valueInputOption="RAW", body={"values": all_data}
    ).execute()
