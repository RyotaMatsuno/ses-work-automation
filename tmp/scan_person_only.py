# -*- coding: utf-8 -*-
"""
過去配信メールから40万以下の【人員情報】のみを送ってきている会社を抽出する
案件情報メールを除外するキーワード判定を追加
"""
...
# 案件系キーワード（これが強く出ていたら案件メールと判定）
# 「募集」「求人」「ご提案ください」など案件側が人を集めているシグナル
...
def is_person_mail(subject, body):
    """人員情報メールかどうか判定"""
    combined = subject + "\n" + body[:1500]

    # 人員キーワードがなければ対象外
    has_person = any(kw in combined for kw in PERSON_KEYWORDS)
    if not has_person:
        return False

    # 案件強キーワードのヒット数をカウント
    project_hits = sum(1 for kw in PROJECT_STRONG_KEYWORDS if kw in combined)

    # 案件キーワードが3個以上 → 案件メールと判定して除外
    if project_hits >= 3:
        return False
    ...
    try:
        ...
        for mid in msg_ids:
            try:
                ...
                # 人員メール判定
                if not is_person_mail(subject, body):
                    continue
...
# ========== 結果出力 ==========
print(f"\n{'=' * 60}")
print(f"完了: チェック{total_checked}件 / 人員メール(40万以下){total_person}件 / {len(sender_data)}社")
print(f"{'=' * 60}")

sorted_senders = sorted(sender_data.items(), key=lambda x: x[1]["count"], reverse=True)

OUTPUT_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "person_only_senders_result.txt")
lines_out = [
    f"40万以下の【人員情報】送信会社一覧\n総計 {len(sorted_senders)}社\nチェックメール数: {total_checked}件\n\n"
]
