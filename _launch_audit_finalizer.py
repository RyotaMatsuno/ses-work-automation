...
def rep(m):
    with open(REP,"a",encoding="utf-8") as f: f.write(f"{datetime.datetime.now():%H:%M:%S} {m}\n")
...
for f in ["cleanup2.py","inject_consts.py","setup_drive_oauth.py"]:
    ...
    if os.path.exists(p):
        try: shutil.move(p,os.path.join(ARCH,f)); rep(f"#11 moved {f}")
        except Exception as e: rep(f"#11 move fail {f}: {e}")
...
for _ in range(160):
    try: sz=os.path.getsize(log3a)
    except: sz=0
    if sz==last and sz>300: stable+=1
...
changed=["mail_attachment_importer/ai_extractor.py","line_webhook/line_query.py",
         "mail_pipeline/mail_pipeline.py","outlook/outlook_to_notion.py",
         "reply_parser/reply_parser.py","mail_attachment_importer/importer.py",
         "mail_attachment_importer/mail_fetcher.py"]
fail=[]
for rel in changed:
    p=os.path.join(BASE,rel)
    if os.path.exists(p):
        r=subprocess.run([sys.executable,"-m","py_compile",p],capture_output=True,text=True,encoding="utf-8",errors="replace")
        rep(f"compile {rel}: {'OK' if r.returncode==0 else 'FAIL'}")
        if r.returncode!=0: fail.append((rel,(r.stderr or '')[:200]))
if fail:
    for rel,e in fail: rep(f"  FAIL {rel}: {e}")
    rep("Phase3a COMPILE FAIL -> commitせず停止。手動レビュー要"); 
    raise SystemExit(0)

# step3: commit 3a
git("add",*[c for c in changed if os.path.exists(os.path.join(BASE,c))],"cost_control")
c=git("commit","-m","cost_control Phase3a: audit fixes F2(guard ai_extractor) F3(freshness created_time) F6(gross floor) F8(FETCH_LIMIT 200) F10(processed-id)")
...
prompt3b=(
 ...
 "(1)engineer_query 内の fetch_all_pages(ENGINEER_DB_ID) を fetch_all_pages(ENGINEER_DB_ID, filter_body={'timestamp':'created_time','created_time':{'on_or_after':'"+d31+"'}}) に変更(エンジニアを直近約1ヶ月に粗絞り)。"
 "(2)project_query 内の fetch_all_pages(ENGINEER_DB_ID) も同様に created_time on_or_after '"+d31+"' を付与。"
 "(3)engineer_query 内の projects 取得(既存_prj_filter rate>0)を {'and':[既存rateフィルタ, {'timestamp':'created_time','created_time':{'on_or_after':'"+d10+"'}}]} に。"
 "(4)project_query 内の projects 取得(既存status=募集中フィルタ)も {'and':[既存statusフィルタ, {'timestamp':'created_time','created_time':{'on_or_after':'"+d10+"'}}]} に。"
...
try:
    r=subprocess.run([CODEX,"exec",prompt3b,"-C",BASE,"--dangerously-bypass-approvals-and-sandbox"],
                     capture_output=True,text=True,encoding="utf-8",errors="replace",timeout=600,cwd=BASE)
...
r=subprocess.run([sys.executable,"-m","py_compile",lq],capture_output=True,text=True,encoding="utf-8",errors="replace")
...
"""
p = os.path.join(WD, "audit_finalizer.py")
with open(p, "w", encoding="utf-8") as f:
    f.write(orch)

# sanity: show current 3a log size, then launch orchestrator detached
sz = (
    os.path.getsize(os.path.join(WD, "cost_control", "codex_phase3a.log"))
    if os.path.exists(os.path.join(WD, "cost_control", "codex_phase3a.log"))
    else 0
)
print("phase3a log size now:", sz)
import subprocess

subprocess.Popen([sys.executable, p], creationflags=0x08000000, cwd=WD)
print("audit_finalizer launched (background, detached)")
