...
def register_to_notion(engineer_info: dict, raw_text: str, subject: str, sender: str) -> bool:
    ...
    properties = {
        "名前": {"title": [{"text": {"content": engineer_info.get("name", "未記載")}}]},
        "稼働状況": {"select": {"name": "稼働可能"}},
        "備考（LINEメモ）": {"rich_text": [{"text": {"content": full_note}}]},
    }

    if "skills" in engineer_info:
        matched_skills = [s for s in engineer_info["skills"] if s in valid_skills]
        if matched_skills:
            properties["スキル"] = {"multi_select": [{"name": s} for s in matched_skills]}
    ...
    data = {"parent": {"database_id": NOTION_ENGINEER_DB_ID}, "properties": properties}
...
def apply_label_and_mark_read(service, msg_id: str, label_id: str):
    ...
    service.users().messages().modify(
        userId="me", id=msg_id, body={"addLabelIds": [label_id], "removeLabelIds": ["UNREAD"]}
    ).execute()
...
def run():
    ...
    # 対象メールを検索
    results = service.users().messages().list(userId="me", q=GMAIL_SEARCH_QUERY, maxResults=50).execute()
