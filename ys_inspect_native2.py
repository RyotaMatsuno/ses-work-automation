...
try:
    ...
    meta = (
        drive.files().get(fileId=SS_ID, fields="name,capabilities(canEdit),driveId", supportsAllDrives=True).execute()
    )
    print(
        f"file={meta.get('name')}  canEdit={meta.get('capabilities', {}).get('canEdit')}  driveId={meta.get('driveId')}",
        flush=True,
    )
except Exception as e:
    print(f"[capability check skipped] {str(e)[:150]}", flush=True)
...
for sh in info["sheets"]:
    if sh["properties"]["title"] == "経験者用":
        ...
        print(f"\n=== 結合セル (R1-29 / 全{len(merges)}件中{len(rel)}件) ===", flush=True)
        for m in rel:
