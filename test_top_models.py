...
print('o3:', r.status_code, r.json().get('choices',[{}])[0].get('message',{}).get('content','') if r.status_code==200 else r.json().get('error',{}).get('message','')[:150])
...
print('Gemini 2.5 Pro:', r2.status_code, r2.json().get('candidates',[{}])[0].get('content',{}).get('parts',[{}])[0].get('text','') if r2.status_code==200 else r2.json().get('error',{}).get('message','')[:150])
