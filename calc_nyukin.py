...
def calc_payment_date(work_year, work_month, site):
    ...
    invoice_end = date(
        invoice_month.year, invoice_month.month, calendar.monthrange(invoice_month.year, invoice_month.month)[1]
    )
    # 入金日 = 請求日 + サイト日数
...
for work_year in [2026, 2027]:
    for work_month in range(1, 13):
        # 2026年4月〜2027年3月のみ
        if (work_year, work_month) < (2026, 4):
            continue
        if (work_year, work_month) > (2027, 3):
            continue

        for s in STAFF:
            if s["type"] == "SKIP":
                continue
            if not is_active(s, work_year, work_month):
                continue
...
rows.append(
    ["※TERRA：請求額(税抜)×10.21%を源泉控除　GL/FT：税込がそのまま実入り", "", "", "", "", "", "", "", "", "", ""]
)
rows.append(
    [
        "入金予定日",
