...
# 請求書リスト取得テスト
res = requests.get(
    f"https://api.freee.co.jp/api/1/invoices?company_id={COMPANY_ID}&limit=5",
    headers=headers
)
...
if res.status_code == 200:
    ...
    for inv in invoices[:3]:
        print(f"  {inv.get('invoice_number')} / {inv.get('partner_name')} / {inv.get('total_amount')}円 / {inv.get('invoice_status')}")
else:
