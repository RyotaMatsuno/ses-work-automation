...
for fname in targets:
    ...
    content = content.replace(
        "deK5gH1TW7wVL20s1Fgfayqb4eKD-iiPyumvPV782uE5cJlYfN8bGl6cu_3m6mrYbt-A8-YWxH2eyI6JXNsvkg", NEW_CLIENT_SECRET
    )
    content = content.replace(
        "deK5gH1TW7wVL2Os1Fgfayqb4eK0-iiPyumvPV782uE5cJIYFN8bGl6cu_3m6mrYbt-A8-YWxH2eyI6JXNsvkg", NEW_CLIENT_SECRET
    )

    # token.jsonのパスを新しいfreee_token.jsonに統一
    content = content.replace('os.path.join(os.path.dirname(__file__), "token.json")', f'r"{NEW_TOKEN_FILE}"')
    content = content.replace("token.json", "freee_token.json")
