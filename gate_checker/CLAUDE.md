...
- 既存 agreement_checker（GPT-4o + Gemini）は触らない（v2.x スコープ外）
...
| 状況 | 挙動 | exit |
|---|---|---:|
| OK + 松野確認不要 | 結果JSON保存 | 0 |
| OK + 松野確認要 | LINE通知「仕様確認お願いします」 | 0 |
| NG + 技術的 | wall_hitting.py 自動実行 → results/保存 | 1 |
| NG + 仕様・致命的 | LINE通知「要確認」 | 1 |
...
- agreement_checker（GPT-4o + Gemini）を書き換えない（v2.x スコープ外）
