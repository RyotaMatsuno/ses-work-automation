...
terra_eng = [
    ...
    ('片山',15000,False,ym(2025,4),ym(2026,8),0),       # ★8月末に延長
    ...
    ('永野',15000,False,ym(2025,10),ym(2026,7),0),       # 7月末終了
    ('安江',15000,False,ym(2025,10),ym(2026,7),0),       # ★7月末終了(両方)
    ...
    ('佐々木BP',40000,False,ym(2026,4),ym(2026,7),20000), # ★7月末終了
...
ft_eng = [
    ...
    ('佐々木FT',27200,False,ym(2026,4),None,0,20000),    # FT佐々木駿は別人→継続
    ('橋本FT',47600,False,ym(2026,4),None,17000,10000),
    ('鶴川FT',47600,False,ym(2026,5),None,47600,0),      # 全額払出
...
# Summary
print(f"\n{'月':>4} | {'TR実入':>8} | {'GL':>7} | {'FT':>7} | {'直契約':>7} | {'岡本':>8} | {'木原':>8} | {'総実入':>9}")
print("-"*80)
ann_sou=0
for pm in pay_months:
...
# Key changes impact
print("\n=== 変動ポイント ===")
print("9月: 佐藤礼奈+稲葉+遠藤の初回入金あり→ピーク")
print("10月: 片山延長で残留、永野安江+佐々木BP退場で減")
print("11月: 片山も終了で更に減、稲葉最終入金")
print("12月~: 安定期(佐々木BP交代要員が入れば回復)")
