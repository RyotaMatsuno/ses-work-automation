...
w("\n=== git diff --stat ===")
r=subprocess.run(["git","-C",BASE,"diff","--stat"],capture_output=True,text=True,encoding="utf-8",errors="replace")
...
if os.path.exists(mc): w(open(mc,encoding="utf-8",errors="replace").read()[:900])
else: w("NOT FOUND")
...
for p in glob.glob(os.path.join(BASE,"**","*.py"),recursive=True):
    rel=p.replace(BASE,"")
    if any(s in rel for s in ["model_config","_archive","test","patch_","rewrite_","build_importer","write_","_audit","fix_","add_","check_","diagnose","poc_","consult_","validate_","cost_estimate","cost_calculator","ledger.py"]): continue
...
for i,l in enumerate(lines,1):
    if "_select_fallback_model" in l: grab=True
    if grab:
        w(f"  {i}: {l.rstrip()[:120]}")
        if i>0 and "return" in l and grab and l.strip().startswith("return") and i> [j for j,x in enumerate(lines,1) if '_select_fallback_model' in x][0]+2:
            pass
...
for rel in ["mail_pipeline/mail_pipeline.py","skill_reader/skill_reader.py","outlook/outlook_to_notion.py"]:
    ...
    w(f"  {rel}: ledger_can_spend={'YES' if 'ledger_can_spend' in t else 'NO'} / VISION_MODEL={'YES' if 'VISION_MODEL' in t else '-'} / TEXT_MODEL={'YES' if 'TEXT_MODEL' in t else '-'}")
