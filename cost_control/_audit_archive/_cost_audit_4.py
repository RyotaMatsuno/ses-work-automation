...
def summarize(path):
    ...
    for ln in open(path,encoding="utf-8",errors="replace"):
        ...
        a["n"]+=1; a["in"]+=d.get("input_tokens",0); a["out"]+=d.get("output_tokens",0); a["usd"]+=d.get("cost_usd",0.0)
        tot+=d.get("cost_usd",0.0)
        if d.get("ts"): days.add(d["ts"][:10])
    return agg,tot,days

for f in ["usage_tracker/cost_log_archive_2026-06.jsonl","usage_tracker/cost_log.jsonl","usage_tracker/cost_log_archive_2026-05.jsonl"]:
    p=os.path.join(BASE,f)
    if not os.path.exists(p): w(f"\n## {f}: NOT FOUND"); continue
    try:
...
for c in cands[:10]: w("  ",c.replace(BASE,""), os.path.getsize(c),"B")
# pick the production one (largest non-test in mail_pipeline dir or root)
prod=None
for c in cands:
    if c.endswith("mail_pipeline.py"): prod=c; break
if not prod and cands: prod=max(cands,key=os.path.getsize)
...
if prod:
    ...
    for kw in ["PROCESS_LIMIT","LIMIT =","配信","出回","一斉","broadcast","スキップ","除外","body[:","本文[:","671","名分","skill_judge","判定"]:
...
if os.path.exists(ip):
    ...
    with open(ip,"rb") as fh: fh.seek(max(0,sz-25000)); t=fh.read().decode("utf-8",errors="replace")
    L=t.splitlines()
    w(f"  size={sz}B  limit-hits-in-tail={sum(1 for l in L if 'limit' in l.lower() or '上限' in l)}")
    for l in L[-10:]: w("   ",l[:150])

# 4. scheduled tasks
w("\n=== scheduled tasks (filtered) ===")
try:
    r=subprocess.run(["schtasks","/query","/fo","csv","/nh"],capture_output=True,text=True,encoding="cp932",errors="replace")
    for line in (r.stdout or "").splitlines():
        if any(k in line for k in ["SES","ses_","freee","usage","Matching","Mail","attachment","watchdog","daily","cost"]):
            w("   ",line[:150])
except Exception as e:
    w("  schtasks ERR",e)
