...
for res in rr.json().get("results", [])[:3]:
    ...
    for k, v in pr.items():
        t = v.get("type")
        if t == "title":
            flat[k] = "".join(x.get("plain_text", "") for x in v.get("title", []))[:30]
        elif t == "status":
            flat[k] = (v.get("status") or {}).get("name")
        elif t == "select":
            flat[k] = (v.get("select") or {}).get("name")
        elif t == "date":
            flat[k] = (v.get("date") or {}).get("start")
        elif t == "created_time":
            flat[k] = v.get("created_time")
