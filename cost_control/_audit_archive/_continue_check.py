...
for p in glob.glob(os.path.join(BASE, "**", "*.py"), recursive=True):
    rel = p.replace(BASE, "")
    if "_archive" in rel or rel.endswith("double_check.py"):
        continue
    try:
    ...
    for i, l in enumerate(t.splitlines(), 1):
        ...
        if s.startswith("#"):
            continue
        if "double_check" in l.lower() and (
            "import" in l or "from" in l or "double_check(" in l.lower() or "run_double" in l.lower()
        ):
            w(f"  {rel}:{i}: {s[:110]}")
            hits += 1
w(f"  -- 参照箇所: {hits}件")
