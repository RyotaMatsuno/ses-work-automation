...
q=subprocess.run(["schtasks","/query","/tn","SES_MailPipeline","/fo","list"],capture_output=True,text=True,encoding="cp932",errors="replace")
...
last_start=None; last_excl=None; last_cls=None; n_excl_total=0
if os.path.exists(mp):
    with open(mp,encoding="utf-8",errors="replace") as f:
        for line in f:
            if "起動" in line or "v5.1" in line: last_start=line.strip()
            if "配信除外" in line: last_excl=line.strip(); n_excl_total+=1
            if "分類対象" in line: last_cls=line.strip()
...
try:
    ...
    if isb is None:
        ...
        cases=[
          ("List-Unsubscribeあり", {"sender":"news@bulk.example.com","headers":{"list-unsubscribe":"<mailto:u@x>"},"body":"案件のお知らせ","to":"a@x"}, True),
          ("フッタに配信停止", {"sender":"info@example.com","headers":{},"body":"案件詳細...\n\n配信停止はこちら https://x/unsub","to":"a@x"}, True),
          ("通常の個別案件", {"sender":"tanaka@bp-partner.co.jp","headers":{},"body":"下記案件いかがでしょうか。Java/AWS、70万、即日","to":"sessales@terra-ltd.co.jp"}, False),
        ]
