...
for p in glob.glob(os.path.join(BASE,"**","*.py"),recursive=True):
    if ".git" in p or "_archive" in p: continue
    ...
    if "def extract_skills_from_image" in t or "def extract_skills_from_text" in t:
        ...
        for i,l in enumerate(lines,1):
            s=l.strip()
            if ("def extract_skills" in s or "model" in l.lower() or "sonnet" in l.lower() 
                or "haiku" in l.lower() or "media_type" in l or "image" in s and "base64" in s):
                if not s.startswith("#"):
                    w(f"  {i}: {s[:130]}")
...
if os.path.exists(p):
    ...
    for i,l in enumerate(lines,1):
        s=l.strip()
        if ("model" in l.lower() or "sonnet" in l.lower() or "haiku" in l.lower() or "claude-" in l) and not s.startswith("#"):
            w(f"  {i}: {s[:130]}")
else:
...
for rel in ["usage_tracker/cost_calculator.py","common/ledger.py"]:
    ...
    for i,l in enumerate(open(p,encoding="utf-8",errors="replace").read().splitlines(),1):
        if ("sonnet" in l.lower() or "haiku" in l.lower() or "gpt" in l.lower() or "price" in l.lower() 
            or "1_000_000" in l or "/ 1000" in l or "input" in l.lower() and "0." in l):
            s=l.strip()
