...
for n, p in props.items():
    ...
    if t in ("status", "select"):
        ...
        w(f"     options: {opts}")
        if any(k in n for k in ["ステータス", "状態", "status", "Status"]):
            status_prop = (n, t, opts)
    if t in ("date", "created_time"):
        date_props.append((n, t))
...
while True:
    ...
    if rr.status_code != 200:
        w("query err", rr.status_code, rr.text[:200])
        break
    d = rr.json()
...
for n, t in date_props:
    if any(k in n for k in ["受信", "登録", "作成", "日付", "date", "Date"]) and t == "date":
        prefer = (n, t)
        break
if not prefer and date_props:
    prefer = date_props[0]
...
for k, v in status_stale.most_common():
    w(f"  {k}: {v}")
