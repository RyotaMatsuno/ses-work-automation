...
for i,l in enumerate(lines):
    if re.match(r"\s*def is_broadcast", l): start=i; break
...
for i,l in enumerate(lines,1):
    if re.search(r"(FETCH_LIMIT|CLASSIFY_LIMIT|PROCESS_LIMIT|BROADCAST_SENDER_ALLOWLIST|RECIPIENT)", l) and not l.strip().startswith("#"):
        w(f"  {i}: {l.strip()[:120]}")
...
with open(os.path.join(BASE,"_verify_phase2.txt"),"w",encoding="utf-8") as f:
    f.write("\n".join(out))
print("DONE lines_pe=", len(open(pe,encoding='utf-8',errors='replace').read().splitlines()) if os.path.exists(pe) else 0)
