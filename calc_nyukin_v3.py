...
for k in sorted(buckets.keys()):
    ...
    if (py,pm)!=cur_ym:
        if cur_ym:
            cy,cm=cur_ym
            t=ym["terra"]; g=round(t*GENTEN); tx=round(t*1.1); ri=tx-g
            tot=ri+ym["gl"]+ym["ft"]-ym["ok"]-ym["kh"]
            rows.append([f"{cy}年{cm}月 小計","",t,g,tx,ri,ym["gl"],ym["ft"],-ym["ok"],-ym["kh"],tot])
            rows.append(["","","","","","","","","","",""])
            annual["terra"]+=t; annual["gl"]+=ym["gl"]; annual["ft"]+=ym["ft"]
            annual["ok"]+=ym["ok"]; annual["kh"]+=ym["kh"]; annual["real"]+=tot
            ym={"terra":0,"gl":0,"ft":0,"ok":0,"kh":0}
        cur_ym=(py,pm)
        rows.append([f"◆ {py}年{pm}月 入金分","","","","","","","","","",""])

    b=buckets[k]
    terra=b["TP"]+b["TB"]; gl=b["GL"]; ft=b["FT"]; ok=b["OK"]; kh=b["KH"]
    g=round(terra*GENTEN); tx=round(terra*1.1); ri=tx-g
    tot=ri+gl+ft-ok-kh
    dd=15 if dt==15 else last_day(py,pm)
    src=", ".join(sorted(set(b["names"])))
    rows.append([f"{py}/{pm:02d}/{dd:02d}",src,
                 terra if terra else "-",g if g else "-",
                 tx if tx else "-",ri if ri else "-",
                 gl if gl else "-",ft if ft else "-",
                 -ok if ok else "-",-kh if kh else "-",tot])
    ym["terra"]+=terra; ym["gl"]+=gl; ym["ft"]+=ft; ym["ok"]+=ok; ym["kh"]+=kh
...
rows.append(["2026年度 年間総合計","",annual["terra"],g,tx,ri,
             annual["gl"],annual["ft"],-annual["ok"],-annual["kh"],annual["real"]])
