...
## pending_tasks 自動実行ルール（2026-06-11追加）

### Cursorを開いたとき
1. `ses_work/pending_tasks/` フォルダを確認する
2. `.md` ファイルが存在する場合、ファイル名の番号順（001→002→...）に実行する
3. 各ファイルの `【Cursor作業指示】` セクションを読み、指示通りに実装する
4. 実装完了後、そのファイルを `ses_work/done_tasks/` に移動する
5. 全ファイル完了後、`ses_work/local_server/task_runner.py` に完了通知を送る

### ファイル命名規則
- `001_タスク名.md` 形式（3桁連番 + アンダースコア + タスク名）
- 例: `001_gate_checker_v3.md`, `002_matching_fix.md`
