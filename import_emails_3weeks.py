...
SKILL_KEYWORDS = [
    ...
    'Node.js', 'Node ', 'React', 'AWS', 'インフラ', 'Linux',
    'ネットワーク', 'プログラマ', 'エンジニア', '要員', '技術者',
]
PRICE_KEYWORDS = [
    '単価', '希望単価', '月額', '/月', '万円', '万/月',
]
START_KEYWORDS = [
    '稼働可能', '参画可能', '開始時期', '入場可能', '稼働日',
    '即日', '翌月', '来月から', '稼働開始',
]
...
def parse_info(subject, body, sender):
    ...
    for pat in [
        ...
        if m:
            ds = m.group(1)
            ds = re.sub(r'(\d{4})年(\d{1,2})月(\d{1,2})日?',
                       lambda x: f"{x.group(1)}-{int(x.group(2)):02d}-{int(x.group(3)):02d}", ds)
...
def main():
    ...
    for i, mid in enumerate(ids):
        try:
            ...
            if ok:
                print(f"  ✅ 登録: {info.get('name', '名前未設定')} / スキル: {info.get('skills', [])} / 単価: {info.get('price', '-')}万")
                registered += 1
                results.append({
                    'name': info.get('name', ''),
                    'subject': subject,
    ...
    with open(out, 'w', encoding='utf-8') as f:
        json.dump({'registered': registered, 'skipped': skipped, 'total': len(ids), 'items': results}, f, ensure_ascii=False, indent=2)
    print(f"結果保存: {out}")
