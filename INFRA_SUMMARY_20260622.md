...
### 分類ロジック v4 (score-based + body analysis)
- analyze_final.py 全面改修
- Subject + Body スコアリング方式に移行
- BODY_ENGINEER_STRONG / BODY_PROJECT_STRONG 導入
- 同スコア時はengineer優先（人員混入防止）
- 300件検証: 93.7%精度 / FALSE PROJECT 0.67%
- EBA/engineer-mikata等の特殊パターン対応

### Notion案件DBクリーンアップ
- 人員混入 90+19件 = 109件を営業終了に変更
- 単価異常値 23件をnullに修正
- 募集中: 602→583件（クリーン）

### マッチング改善デプロイ
- Cloud Run revision line-webhook-00080-ctn
- PHさん: 0件→5件マッチ（人員混入なし確認済み）

### スケジューラ修正
- jobz_matching_daily: 無効→有効化（次回6/23 8:00）
- SES_Freee_Invoice: 無効→有効化（次回6/25 10:00）

### Task Q: 抽出精度改善（Cursor実行中）
- price_extractor.py: Rule-first単価抽出
- skill_extractor.py: Header regex + 辞書スキャン
- 目標: No skills 56%→35%以下、No price 48%→25%以下

### Investigation R: 全システム徹底調査（Cursor実行中）
- 全コードベース7カテゴリ21項目の調査
- INVESTIGATION_REPORT.md に出力予定


### Task T: P0/P1修正（2026-06-22 完了）
- P0-1: CostGuard TOCTOU → reservation保持
- P0-2: CostGuard fail-open → fail-close
- P0-4: Cloud Run import → importlib明示読込
- P0-5: matching ANY → ALL-required (#skill_skip維持)
- P1-2: Batch classify body[:100] → [:2000]
- P1-7: nightly_jobz NameError修正
- P1-16: skill_extractor誤マッピング削除
- P1-17: 辞書スキャン条件修正
- テスト: 38/38 passed

### 残セキュリティタスク（後回し）
- P0-3: IMAP TLS証明書検証
- P0-7: command_server RCE対策（allowlist + shell=False）
- P0-8: freee OAuth client_secret env移行
