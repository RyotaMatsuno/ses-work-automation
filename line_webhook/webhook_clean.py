...
{"candidates":[{"name":"","price":0,"summary":"","required_match":{},"optional_match":{},"parallel":"なし","engineer_source":"松野 or 岡本 or 不明"}],"proposal_draft":""}"""
    result = call_claude(
        system, json.dumps({"project": project, "engineers": engineers}, ensure_ascii=False), max_tokens=2000
    )
    try:
        return json.loads(re.sub(r"```json|```", "", result).strip())
    except:
...
def register_project(info: dict, raw_text: str, sender: str) -> tuple:
    ...
    if info.get("start_date"):
        props["開始日"] = {"date": {"start": info["start_date"]}}
...
def get_available_engineers() -> list:
    ...
    for p in pages:
        ...
        source = "不明"
        if "LINEから自動登録: 松野" in note:
            source = "松野"
        elif "LINEから自動登録: 岡本" in note:
            source = "岡本"
        result.append(
            {"name": name, "skills": skills, "price": price, "note": note[:300], "notion_id": p["id"], "source": source}
        )
    return result
        ...
        if intent_owner != sender and intent_owner != "両方":
            ...
            for person in ["松野", "岡本"]:
                if person != sender:
                    ...
                    push_message(
                        uid, f"📋 {sender}から案件「{proj_name}」が来ました\n担当候補者の確認をお願いします", tok
                    )
