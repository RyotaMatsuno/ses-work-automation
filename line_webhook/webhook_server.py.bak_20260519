...
IMPORTANT: Ignore forwarding remarks and extract only actual job/engineer info.
IMPORTANT: price field must be in 万 unit as integer. e.g. "65万"->65, "45万〜50万"->47, "650,000円"->65.
...
def evaluate_candidate(candidate, project_price):
    ...
    if cp == 0:
        ng_reasons.append("単価未設定")
    elif pp > 0:
        diff = pp - cp
        if diff < 5:
            ng_reasons.append(f"粗利{diff}万（最低5万未満）")

    required_match = candidate.get("required_match", {})
    missing = [k for k, v in required_match.items() if not v]
    if missing:
        ng_reasons.append(f"必須NG: {', '.join(missing)}")
